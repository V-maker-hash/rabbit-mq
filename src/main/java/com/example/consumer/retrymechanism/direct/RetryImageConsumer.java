package com.example.consumer.retrymechanism.direct;

import com.example.consumer.retrymechanism.DlxProcessingErrorHandler;
import com.example.entity.Picture;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.rabbitmq.client.Channel;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.amqp.core.Message;
import org.springframework.amqp.rabbit.annotation.RabbitListener;
import org.springframework.amqp.support.AmqpHeaders;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.messaging.handler.annotation.Header;
import org.springframework.stereotype.Service;

@Service
public class RetryImageConsumer {

    private static final Logger LOGGER = LoggerFactory.getLogger(RetryImageConsumer.class);

    private static final String DEAD_EXCHANGE_NAME = "x.guideline.dead";

    private final DlxProcessingErrorHandler dlxProcessingErrorHandler;

    public RetryImageConsumer() {
        dlxProcessingErrorHandler = new DlxProcessingErrorHandler(DEAD_EXCHANGE_NAME);
    }

    @Autowired
    private ObjectMapper objectMapper;

    @RabbitListener(queues = "q.guideline.image.work")
    public void getMessageFromPictureRetryDirectProducer(Message message, Channel channel,
                                                         @Header(AmqpHeaders.DELIVERY_TAG) long tag) {
        try {
            var picture = objectMapper.readValue(message.getBody(), Picture.class);
            if (picture.getSize() > 9000) {
                throw new IllegalArgumentException("Picture is too large");
            } else {
                LOGGER.info("Creating and publishing: {}", picture);
                channel.basicAck(tag, false);
            }
        } catch (Exception e) {
            var s = new String(message.getBody()) + " : " + e.getMessage();
            LOGGER.warn("Error processing message : {}", s);
            dlxProcessingErrorHandler.handleErrorProcessingMessage(message, channel, tag);
        }


    }
}
